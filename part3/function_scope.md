...
雖然上一節有說過的，有個隱藏的`arguments`物件，它可以獲取到所有傳入的參數值，然後用類似陣列的方式來使用，但它的設計相當怪異(有一說是設計錯誤)，使用時要注意很多例外的情況，加上使用前根本也不需要定義，很容易造成程式碼閱讀上的困難。另外，在一些測試報告中，使用`arguments`物件本身比有直接使用具有名稱傳入參數慢了數倍。所以結論是，`arguments`物件的是不建議使用它的。
