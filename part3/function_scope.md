...
- 給定一樣的輸入(傳入值)，一定會回傳相同輸出值結果(return)
- 不會產生副作用
- 不依賴任何外部的狀態
...
套用上面的定義，你可以這樣理解，所以它是一個純粹函式:

- 只要每次給定相同的輸入值(例如1與2)，就一定會得到相同的輸出值(3)
- 不會改變輸入參數，所以沒有副作用
- 不依頼其他外部的狀態(變數之類的)

那什麼又是一個不純粹的函式？看以下的範例就是，它主要需要依賴外部的狀態值(變數值):
...
在JavaScript不純粹的函式很多，在本書中一直用來作為輸出的`console.log`函式，或是在範例程式常用的`alert`函式，都是不純粹函式，這類函式因為沒有回傳值，用來作某件事而已，像`console.log`會更動瀏覽器的主控台(函式外部環境)的輸出，也算一種副作用。

還有許多內建的或常見的函式都是具有副作用的，例如這些應用:
...
而每次輸出值都不同的，最典型的就是`Math.random`，這是產生隨機值的內建函式，既然是隨機值當然每次執行的回傳值都不一樣，這也是不純粹函式的一類。

總之，並不是說有副作用的函式就不要使用，而且要知道這個概念，然後多在你自己的寫的函式上使用純粹函式，而且讓具有副作用的函式得到控制，有些新的函式庫或框架要求在某些地方只能使用純粹函式，在某些地方才能使用具有副作用的不純粹函式，使用時就要特別注意了。

純粹函式具有以下的優點:

- 程式碼簡單化，閱讀性提高
- 具有強健(robust)性，重覆使用性高
- 易於單元測試(Unit Test)、除錯
- 組織出不可改變的資料結構(immutable data structures)
