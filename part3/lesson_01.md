...
- (google)使用NAMES_LIKE_THIS這樣的命名方式，來命名常數。(註: 有了const後與變數一樣的方式即可，以前會這樣作，是因為只用var無法分辨是常數還變數)
...
註解(Comment)/康門/的意思是要讓未來的自己或其他人很快的理解，這段程式碼是在作什麼用的。所以在重要的程式碼的地方(通常在上面)，加上註解是好的習慣。註解也常常用在暫時性的把某段程式碼先擋住，讓它不執行。好的註解習慣的養成建議:
