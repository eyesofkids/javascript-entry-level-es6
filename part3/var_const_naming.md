...
變數(Variable)在JavaScript語言中扮演了重要的資料(值)存放角色地位。在ES6標準之前，只有變數沒有常數的設計，現在這兩者有不同的定義關鍵字與設計。
...
- (airbnb 22.4) 避免在命名的前後使用下底線(_)，例如`\_\_firstName\_\_`、`\_firstName`、`firstName\_`都是很糟的命名
...
這裡說的保留字，在使用上是用"全部小寫"的英文單字，例如`let`不能寫成"lEt", "LET", "Let"。

## 總結

本章主要說明了兩個主要的宣告關鍵字`let`與`const`的用法，以及在JavaScript中的命名規則，這些命名規則通用於所有的類別、函式、變數與常數的識別符。
