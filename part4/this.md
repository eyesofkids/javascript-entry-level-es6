...
按照上面章節的說明，在一般情況，也就是在全作用域中定義中的函式，它的this值預設是在瀏覽器中是window物件，在Node.js中是global物件，也就是全域(全局)，這只能適用在非嚴格模式(non-strict mode)的情況。
...
有一個特別的例子是函式中的隱藏物件 - `arguments`物件，在實際上也隱藏了實體化的過程，由JavaScript自動實體化這個物件，當然，它對應上面的陣列與物件的設計，預設裡面的成員如果是個函式是也是自動指向自己本身。如下面的例子:
...
不過用`arguments`物件容易產生誤解，這也是其中一個常見的陷阱，上面的例子中，直接呼叫傳入的函式與使用arguments物件來呼叫，會有不同的結果。這一點是需要特別注意的。

> 特別注意: 在JavaScript中，不要使用`new Array()`或`new Object()`來建立陣列與物件的實體(實例)，而應該使用`[]`或`{}`的定義方式。
